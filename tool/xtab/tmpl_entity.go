...
type {{if notblank .Master}}{{camel .Slave}}{{end}}Entity struct {
	...
	{{padname $column.Field $.FieldMaxLen}} {{padname $column.Etype $.TypeMaxLen}} ` + "`" + `db:"column={{$column.ID}}&comment={{$column.Name}}{{if notblank $column.DictGenre}}&dict={{$column.DictGenre}}{{end}}{{if notblank $column.Defaults}}&default={{$column.Defaults}}{{end}}{{if not $column.IsDisplay}}&display=false{{end}}"` + "`" + `{{end}}{{end}}
}
`
